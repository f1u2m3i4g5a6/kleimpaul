...
(()=>{
  ...
  const SECTIONS=[
    ...
    ['orders','Orçamentos / O.S.','Oficina, máquinas, peças e serviços','fa-screwdriver-wrench'],
    ['clients','Clientes','Cadastro, busca e histórico de clientes','fa-users'],
  ...
  function groupSidebar(){
    ...
    before('dashboard','Visão geral');before('editor','Conteúdo');before('inventory','Operação');before('orders','Oficina');before('clients','Clientes & comercial');before('backup','Segurança');before('settings','Sistema');
  }
  ...
  function installSidebarCollapse(){
    ...
    if(!sidebar||!brand||$('#sidebarCollapse'))return;
    const btn=document.createElement('button');
    btn.id='sidebarCollapse';
    btn.className='sidebar-collapse-btn';
    btn.type='button';
    btn.setAttribute('aria-label','Minimizar menu lateral');
    btn.innerHTML='<i class="fa-solid fa-minus"></i>';
    brand.appendChild(btn);
    const apply=(collapsed,save=true)=>{
      document.body.classList.toggle('sidebar-collapsed',!!collapsed);
      btn.innerHTML=collapsed?'<i class="fa-solid fa-plus"></i>':'<i class="fa-solid fa-minus"></i>';
      btn.title=collapsed?'Expandir menu':'Minimizar menu';
      btn.setAttribute('aria-label',btn.title);
      btn.setAttribute('aria-expanded',collapsed?'false':'true');
      if(save){try{localStorage.setItem('kleimpaul_sidebar_collapsed',collapsed?'1':'0')}catch{}}
    };
    let initial=false;try{initial=localStorage.getItem('kleimpaul_sidebar_collapsed')==='1'}catch{}
    apply(initial,false);
    btn.addEventListener('click',()=>apply(!document.body.classList.contains('sidebar-collapsed')));
    const setTitles=()=>$$('#nav button[data-go]').forEach(b=>{const label=$('span',b)?.textContent?.trim();if(label)b.title=label;});
    setTitles();setTimeout(setTitles,120);
  }
  ...
  function init(){installTopSearch();installQuickActions();groupSidebar();installSidebarCollapse();keyboard();}
